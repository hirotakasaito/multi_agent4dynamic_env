...
from multi_gym_sfm.envs.cython.utils import affine_trans, check_EPS_vector, calc_theta
...
# cpdef list select_generate_ac_zone(list zones, int total_step, np.ndarray ag_poses):
cpdef list select_generate_ac_zone(list zones, int total_step, list ag_poses):
    cdef :
        int i = 0
        int sz_i = 0
    ...
    for i, z in enumerate(zones) :
        if not z.agent_only :
            ...
            if z.can_generate_actor :
                for ag_pose in ag_poses:
                    if z.safety_check(ag_pose.pose) : az_index.append(i)
    ...
    if len(az_index) > 0 : list(set(az_index))
